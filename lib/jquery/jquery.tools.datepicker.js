 ...
 * 
 * This version edited to select weeks - combine features later!
 * 
 */
(function($) {	
...
//}}}
	...
	function Dateinput(input, conf)  { 
		...
		//value = zokemDateObject(document.tempDateRange);
...
//{{{ pick
		...
		function select(date, weekStartDate, weekEndDate, conf, e) {
			// quick and dirty handling of month
			if (conf.month) {
				input.val(date + " " + $("#caltitle").text());
				...
				input.data("date", date + " " + $("#caltitle").text());
			}
			else {
				// current value
				value = date;
				...
				if (conf.week) {
					input.val(format(weekStartDate, conf.format, conf.lang) + " - " + format(weekEndDate, conf.format, conf.lang));
				}
				else {
					input.val(format(date, conf.format, conf.lang));
				}
					...
					if (conf.month) {
					 ...
					 if (conf.month) {
					  ...
					  if (year == new Date().getFullYear()) {
					  	monthlimit = new Date().getMonth();
					  }
					  else {
					  	monthlimit = 11;
					  }
					  for (i = 0; i <= monthlimit; i++) {
					  	$("#calbody").append("<div class='calmonth'>" + labels.months[i] + "</div>");
					  }	
								if (min && currYear <= min.getFullYear()) {
									$("#calprev").addClass(css.disabled);
								}
								
								if (max && currYear >= max.getFullYear()) {
									$("#calnext").addClass(css.disabled);
								}
								$("div.calmonth:eq(" + (currMonth) + ")").attr("id", "calcurrent");
						// month picking					
						$(".calmonth").click(function(e){
							var el = $(this);
								$(".calmonth").removeAttr("id");
							if (!el.hasClass(css.disabled)) {
//								$("#" + css.current).removeAttr("id");
								el.attr("id", css.current);
								var monthDate = new Date($("#caltitle").text(),$(this).parent().children(".calmonth").index(this) + 1);
								select(monthDate, monthDate, monthDate, conf, e);
							}
							return false;
						});
					 }
...
//										}
										// current date
										var selectedDate = zokemDateObject(document.tempDateRange);
								...
								a.click(function(e){
									var el = $(this);
									var weekStartEl = $(this).parent().children().first();
									var weekEndEl = $(this).parent().children().last();
									if (!el.hasClass(css.disabled)) {
										
										if (conf.week) {
											$(".currentweek").removeClass("currentweek");
											$("#" + css.current).removeAttr("id");
											el.parent().addClass("currentweek");
										}
										else {
											$("#" + css.current).removeAttr("id");
											el.attr("id", css.current);
										}
										select(el.data("date"), weekStartEl.data("date"), weekEndEl.data("date"), conf, e);
									}
									return false;
								});
							}
						...
						//set hover class for week select, and highlight curret week
						if (conf.week) {
							$(".calweek").hover(function() {
								$(this).children().addClass('calweekhover');
							}, function() {
								$(this).children().removeClass('calweekhover');
							});
							// current week
							$(".calfocus").parent().addClass("currentweek");
						}
				return self;
			}, 
	//}}}
