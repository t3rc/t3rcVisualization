// initiate variables
    ...
    var imageUrlplace = 'http://smartmob.zokem.com/cv/img/icons/bluedot.png'; 
    var imageUrlphone = 'https://chart.googleapis.com/chart?chst=d_fnote_title&chld=pinned_c|1|004400|l|Phone call|you talked here |`with someone?&ext=.png';
...
function addMarkersFromXML(xml) {
		...
		//clear array of markers and infowindows
		markers = [];
		infoWindows = [];
		points = [];
		...
		$(xml).find("event").each(function(){
			...
			var category = $("#MashCategories option:selected").text();
			
			
			var latLng = new google.maps.LatLng(parseFloat(lat), parseFloat(lon));
			
			//details to display in pop-up
			var timestamp = "<b>Time:</b>"+ time + '<br/>';
			var address = "<b>Address:</b> "+address+ '<br/>';
			var htmlString = timestamp +  address; 
			
			
			//if ($("#MashCategories").val() == 'application')
			//{
				$(this).find("customvalue").each(function()
  				{
  					htmlString = htmlString + '<b>'+ $(this).attr("name") +':</b>' + $(this).attr("value")+'<br/>';
  				});
				
			//}
			
			var htmlStringFirst = '<b>'+category+'</b><br/>'+htmlString;
			...
			{
				points.push(latLng);
				infoWindows.push(htmlStringFirst);
		...
		//link the markers to infowindows
		
		if ($("#MashCategories option:selected").val() == 'places' )
		{
			LinkMarkersToInfoWindows(imageUrlplace,
						 new google.maps.Size(24, 32));
		}
		else if ($("#MashCategories option:selected").val() == 'phone' )
		{
			LinkMarkersToInfoWindows(imageUrlphone, 
						 new google.maps.Size(120, 120));
		}
		else
		{
			LinkMarkersToInfoWindows(imageUrl,
						 new google.maps.Size(24, 32));
		}
		...
		{
			point = new google.maps.LatLng(60.1698, 24.9382);
		}
		else
		{
...
function LinkMarkersToInfoWindows(markerIcon, size)
{
	...
	for(var i=0; i<points.length; i++) 
	{
		  ...
		  //var currentInfo = infoWindows[i];					 
		  
			
			//infowindow.open(map);
			
		   //google.maps.event.addListener(marker, 'click', 	displayInfo(infoWindows[i], points[i], this));
		   //});
		  
		  //alert(points[i].lat()+", "+ points[i].lng()+"->"+infowindow.content);
...
{
	infoWindows[i] = infoWindows[i] + "<br/>" + contentHTML;
